// ============================================================
// FARM FRESH RN v4 — PhoneEntryScreen
// Zomato-style auth · Dark glass · Animated keyboard-aware
// ⚠️  No template literals in useAnimatedStyle — plain ternary
// ============================================================
...
import { Colors, Glass, Shadow, Font, TextStyle, Spacing, Radius, Gradient } from '../../theme';
...
const BgBlobs = () => (
  <View style={StyleSheet.absoluteFill} pointerEvents="none">
    ...
    <Animated.View style={styles.blobTopRight} />
    {/* Bottom-left subtle blob */}
    <Animated.View style={styles.blobBottomLeft} />
  </View>
);
...
export const PhoneEntryScreen = ({ navigation }: PhoneEntryScreenProps) => {
  ...
  const handleFocus = useCallback(() => {
    setIsFocused(true);
    inputBorderOpacity.value = withTiming(1, { duration: 220 });
  }, []);

  const handleBlur = useCallback(() => {
    setIsFocused(false);
    inputBorderOpacity.value = withTiming(0, { duration: 220 });
  }, []);
  ...
  const handleSend = useCallback(() => {
    ...
    setIsLoading(true);
    // TODO: Call Firebase Phone Auth / your OTP API here
    setTimeout(() => {
      setIsLoading(false);
  ...
  // ── Animated styles ──────────────────────────────────────
  // ⚠️  No template literals — plain ternary for borderColor
  const inputContainerStyle = useAnimatedStyle(() => ({
    borderColor: inputBorderOpacity.value > 0.5
        ...
        >
          ...
          <Animated.View style={[styles.header, headerStyle]}>
            ...
            <Animated.Text style={styles.subtext}>
              मंडी भाव पर सीधे घर तक — Bhagalpur, Bihar
            </Animated.Text>
          </Animated.View>
          ...
          <Animated.View style={[styles.card, cardStyle]}>
            <View style={styles.cardInner}>
              ...
              <View style={styles.cardLabelRow}>
                ...
                <Animated.Text style={styles.cardLabel}>
                  Enter mobile number
                </Animated.Text>
              </View>
              ...
              >
                <LinearGradient
                  colors={isValid
                    ...
                    : ['rgba(45,138,78,0.35)', 'rgba(45,138,78,0.25)']}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 1 }}
                  style={[
                    styles.ctaButton,
                    isValid && styles.ctaButtonActive,
                  ]}
                >
                  {isLoading ? (
                    ...
                    <View style={styles.ctaRow}>
                      ...
                      <IconChevronRight
                        size={18}
                        color={isValid ? Colors.white : Colors.textOnDarkMuted}
                        strokeWidth={2.5}
                      />
                    </View>
                  )}
          ...
          {/* ── Terms ─────────────────────────────────── */}
          <Animated.Text style={[styles.terms, headerStyle]}>
            By continuing, you agree to our{' '}
            <Animated.Text style={styles.termsLink}>Terms of Service</Animated.Text>
            {' '}and{' '}
            <Animated.Text style={styles.termsLink}>Privacy Policy</Animated.Text>
          </Animated.Text>
...
// ── Constants ─────────────────────────────────────────────
const TRUST_ITEMS = ['Direct mandi price', 'Same-day delivery', 'No middlemen'];
...
const styles = StyleSheet.create({
  ...
  blobBottomLeft: {
    ...
    height: 180,
    borderRadius: 90,
    backgroundColor: 'rgba(128,239,128,0.06)',
  },
});
